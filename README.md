...
B2B E-Commerce & Analytics Platform — публичная реконструкция
backend-системы, разработанной в рамках коммерческого проекта.

Репозиторий создан отдельно от закрытого production-решения и воспроизводит
его основные технические идеи: структуру B2B-каталога, обработку заявок,
server-side tracking, аналитические агрегаты и rule-based скоринг лидов.
...
Подсистема управления заявками: роли сотрудников, назначение, циклы обработки,
комментарии, контакты, история, SLA и внутренние уведомления.
Контракт REST API, миграции и команды запуска: [управление заявками](docs/lead-management.md).
API готовых страниц `/api/v1/crm/`: [контракты CRM](docs/crm-api.md).
Сроки обработки, приоритет по умолчанию `medium` и фоновый запуск:
[SLA и напоминания](docs/sla-reminders.md).
Состояния скоринга, история поведения и показатели кабинета:
[существующие данные CRM](docs/crm-existing-data.md).
Исходный аудит и результаты проверок: [этапы реализации](docs/lead-management-stages.md).
...
Новые заявки и изменения данных для скоринга сохраняют запрос расчёта в
`LeadScoringState`. Обработчик запускается отдельно, без брокера:

```bash
docker compose exec -T web python manage.py score_leads --pending --limit 100
```

Рекомендуемый запуск — раз в минуту; [пример cron](ops/cron/lider-crm.example).
Ошибки не удаляют последнюю оценку и не блокируют обработку заявки.
`--failed` повторяет неудачные расчёты, `--lead-id ID` и `--all` принудительно
пересчитывают выбранную заявку или все заявки. Правила оценки остаются прежними.
